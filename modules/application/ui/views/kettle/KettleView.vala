...
namespace Boiler.Application.UI.Views.Kettle
{
	public class KettleView: BaseView
	{
		...
		#if WITH_UNITY_API
		private Unity.LauncherEntry launcher_entry = Unity.LauncherEntry.get_for_desktop_id(Config.PROJECT_NAME + ".desktop");
		#endif

		public KettleView(DBusKettle kettle)
		{
		...
		construct
		{
			...
			if(kettle.pairing_info != null && kettle.pairing_info.length > 0)
			{
				pairing_info = new InfoBar();
				pairing_info.get_style_context().add_class(Gtk.STYLE_CLASS_FRAME);
			...
			Boiler.Application.Application.instance.toggle_kettle.connect(toggle_kettle);
		...
		private void toggle_kettle()
		{
			...
			kettle.toggle();
			Boiler.Application.Application.instance.kettle_toggle_pending = false;
		}

		private void toggle_kettle_if_pending()
		{
			if(kettle.is_ready && kettle.is_paired && Boiler.Application.Application.instance.kettle_toggle_pending)
			{
				Timeout.add(100, () => {
					if(Boiler.Application.Application.instance.kettle_toggle_pending) toggle_kettle();
		...
		private void update()
		{
			...
			Idle.add(() => {
				...
				lock(kettle)
				{
					((DBusProxy) kettle).set_cached_property("Name", null);
					((DBusProxy) kettle).set_cached_property("Description", null);
					((DBusProxy) kettle).set_cached_property("Status", null);
					((DBusProxy) kettle).set_cached_property("PairingInfo", null);
					((DBusProxy) kettle).set_cached_property("IsPaired", null);
					((DBusProxy) kettle).set_cached_property("IsConnected", null);
					((DBusProxy) kettle).set_cached_property("IsReady", null);
					((DBusProxy) kettle).set_cached_property("IsBoiling", null);
					((DBusProxy) kettle).set_cached_property("Temperature", null);
