...
public class RK_G2XXDevice: Boiler.Devices.Abstract.BTKettle
{
	...
	public RK_G2XXDevice(Bluez.Device device, Bluez.Manager btmgr)
	{
		...
		var key = RK_G2XXSettings.get_instance().auth_key;

		if(key != "")
		{
			if(!Boiler.Converter.hex_to_bin(key, out auth_key, ' '))
			{
	...
	private uint8[] send_command(Command command, uint8[] args)
	{
		...
		lock(counter)
		{
			...
			try
			{
				...
				log("-> %s(%u): %s".printf(command.name(), cmd_index, Boiler.Converter.bin_to_hex(bytes, ' ')));
				cmd_char.write_value(bytes, _params);

				if(!is_connected || cmd_char == null || res_char == null) return {};

				var response = res_char.read_value(_params);
				log("<- %s(%u): %s".printf(command.name(), cmd_index, Boiler.Converter.bin_to_hex(response, ' ')));
				return response;
			}
			catch(Error e)
			{
